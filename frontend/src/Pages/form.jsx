...
export default function Form() {
    ...
    const { register, handleSubmit, formState: { errors } } = useForm({ resolver: zodResolver(blogSchema),mode:"onChange" });
                ...
                <div className="w-screen h-screen flex justify-center items-center">
                    <div ref={formref} className="bg-white1 backdrop-blur-[40px] rounded-[10px] p-[40px] max-500w:p-[27px] max-400w:p-[18px] max-330w:p-[12px] border-1 border-white/20 shadow-auth z-[20]">
                        <form className="flex flex-col w-[470px] max-700w:w-[430px] max-600w:w-[370px] max-500w:w-[335px] max-400w:w-[280px] max-330w:w-[220px] z-[20]" onSubmit={handleSubmit(buttonClickHandler, handleErrors)}>
                            ...
                            <input ref={input1Ref} {...register('name')} id="name" type="text" className={(errors.name) ? "text-white rounded-[15px] border-1 border-red-700 shadow-button p-[10px] w-full z-[20] pointer-events-auto outline-none" : (namecorrect) ? "text-white rounded-[15px] border-1 border-green-400 shadow-button p-[10px] w-full z-[20] pointer-events-auto outline-none" : "text-white rounded-[15px] border-1 border-white/20 shadow-button p-[10px] w-full z-[20] pointer-events-auto outline-none"} placeholder="Enter your name..." onInput={nameinputChange} onBlur={liveValidation} /><AnimatePresence>
                            {errors.name && nameerror && (<motion.p initial={{ opacity: 0, y: -15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -15 }} className="text-red-700 font-bold">{errors.name.message}</motion.p>)}</AnimatePresence>
                            <br />
                            ...
                            <input ref={input2Ref} {...register('heading')} id="title" type="text" className={(errors.heading) ? "text-white rounded-[15px] border-1 border-red-700 shadow-button p-[10px] w-full z-[20] pointer-events-auto" : (headingcorrect) ? "text-white rounded-[15px] border-1 border-green-400 shadow-button p-[10px] w-full z-[20] pointer-events-auto" : "text-white rounded-[15px] border-1 border-white/20 shadow-button p-[10px] w-full z-[20] pointer-events-auto"} placeholder="Enter the title of blog..." onInput={headinginputChange}  onBlur={liveValidation} /><AnimatePresence>
                            {errors.heading && headingerror && (<motion.p initial={{ opacity: 0, y: -15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -15 }} className="text-red-700 font-bold">{errors.heading.message}</motion.p>)}</AnimatePresence>
                            <br />
                            ...
                            <textarea ref={input3Ref} {...register('content')} id="content" className={(errors.content) ? "text-white rounded-[15px] border-1 border-red-700 shadow-button p-[10px] w-full z-[20] pointer-events-auto" : (contentcorrect) ? "text-white rounded-[15px] border-1 border-green-400 shadow-button p-[10px] w-full z-[20] pointer-events-auto" : "text-white rounded-[15px] border-1 border-white/20 shadow-button p-[10px] w-full z-[20] pointer-events-auto"} placeholder="Write the blog here..." onInput={contentinputChange} onBlur={liveValidation} /><AnimatePresence>
                            {errors.content && contenterror && (<motion.p initial={{ opacity: 0, y: -15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -15 }} className="text-red-700 font-bold">{errors.content.message}</motion.p>)}</AnimatePresence>
                            <br />
