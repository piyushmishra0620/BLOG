            ...
            <div className="min-w-screen min-h-screen bg-linear-120 from-10% from-auth-left via-30% via-auth-right to-auth-left overflow-hidden">
                ...
                <div className="w-screen h-screen flex justify-center items-center">
                    <div ref={formref} className="bg-white1 backdrop-blur-[40px] rounded-[10px] p-[40px] max-500w:p-[27px] max-400w:p-[18px] max-330w:p-[12px] border-1 border-white/20 shadow-auth z-[20]">
                        <form className="flex flex-col w-[470px] max-700w:w-[430px] max-600w:w-[370px] max-500w:w-[335px] max-400w:w-[280px] max-330w:w-[220px] z-[20]" onSubmit={handleSubmit(buttonClickHandler)}>
                            ...
                            <input ref={input1Ref} {...register('name')} id="name" type="text" className="text-white rounded-[15px] border-1 border-white/20 shadow-button p-[10px] w-full z-[20] pointer-events-auto" placeholder="Enter your name..." />
                            {errors.name && (<p className="text-red-700 font-semibolg">{errors.name.message}</p>)}
                            <br />
                            ...
                            <input ref={input2Ref} {...register('heading')} id="title" type="email" className="text-white rounded-[15px] border-1 border-white/20 shadow-button p-[10px] w-full z-[20] pointer-events-auto" placeholder="Enter the title of blog..." />
                            {errors.heading && (<p className="text-red-700 font-semibold">{errors.heading.message}</p>)}
                            <br />
                            ...
                            <textarea ref={input3Ref} {...register('content')} id="content"  className="text-white rounded-[15px] border-1 border-white/20 shadow-button p-[10px] w-full z-[20] pointer-events-auto" placeholder="Write the blog here..." />
                            {errors.content && (<p className="text-red-700 font-semibold">{errors.content.message}</p>)}
                            <br />
                            <button type="submit" className="text-white text-1xl font-sans font-semibold py-4 w-full rounded-[15px] mt-10 border-1 border-white/20 cursor-pointer shadow-button hover:bg-cyan-300 hover:text-black hover:font-semibold hover:text-1xl hover:font-sans hover:-translate-y-1 hover:shadow-cyan-400 hover:shadow-sm active:animate-click active:bg-cyan-300 duration-200 ease-in-out z-[20] pointer-events-auto">Create Blog</button>
                        </form>
                    </div>
                </div>
            </div>
